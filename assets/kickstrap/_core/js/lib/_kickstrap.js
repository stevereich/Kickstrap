...
var cursorSpot = 0,
  ...
  , mouseIn = false
  , kbash = new Object()
  , appKeychain = new Array()
  , app = function(name, errors) {
    this.name = name
    this.errors = errors
    appKeychain.push(this)
  }
...
define(['jquery'],
  function($) {
    ...
    kbash.less = function(args, flags, opts, props) {
      if (props.empty) { console.kbash('Please enter a command') }
      if (args.have('watch')) { 
        try {
          less.watch()
          console.kbash('<span class="green">WATCH MODE</span>: Enabling LESS watch mode')
        }
        catch(e) {
          console.kbash('<span class="red">ERROR:</span> Could not call less.watch()')
          console.kbash(e)
        }
      }
    }
